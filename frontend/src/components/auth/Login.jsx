...
const Login = () => {
  ...
  const submitCredentials = async () => {
    ...
    try {
      ...
      if (resp.data.mfaRequired) {
        console.log("🔒 MFA required for user:", resp.data.user);
        setMfaRequired(true);
        setMfaUser(resp.data.user);
      } else if (resp.data.success) {
        const { token, user } = resp.data;
        console.log("✅ Direct login successful - User:", user);
        console.log("📋 User details - EmployeeId:", user.employeeId, "Role:", user.role);
  ...
  const submitOtp = async () => {
    ...
    try {
      ...
      console.log("🔓 MFA verification response:", resp.data);
      
      if (resp.data.success) {
        const { token, user } = resp.data;
        console.log("✅ MFA verification successful - User:", user);
        console.log("📋 User details - EmployeeId:", user.employeeId, "Role:", user.role);
